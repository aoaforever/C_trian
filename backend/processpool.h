...
private:
    ...
    processpool(int listenfd, int process_number=8);//没有详细写函数，在后面才写
public:
...
static void addfd(int epollfd, fd){
    epoll_event event;
    event.data.fd = fd;
...
//handler不应该是函数指针吗？
statci void addsig(int sig, void(handler)(int), bool restart = true){
    struct sigaction sa;
...
/*进程池构造函数，参数listenfd是监听socket，它必须在创建进程池之前被创建，否则
...
m_idx(-1), m_stop(false){
    ...
    //创建process_number 个子进程，并建立它们和父进程之间的管道
    for(int i =0; i<process_number;++i){
        int ret = socketpair(PF_UNIX, SOCK_STREAM, 0, m_sub_process[i].m_pipefd);
        assert(ret == 0);

        m_sub_process[i].m_pid = fork();//如果是0，表示子进程;pid表示父进程
        assert(m_sub_process[i].m_pid >=0);
        if(m_sub_process[i].m_pid>0){
            close(m_sub_process[i].m_pipefd[1]);//父进程关闭写,实际上并不是，因为socketpair一个fd就能双向
            continue;
        }
        else{
...
/*统一事件源*/
template<class T>
void processpool<T>::setup_sig_pipe(){//每个子进程都有一个epoll内核事件表
    //创建epoll事件监听表和信号管道
...
void processpool<T>::run_child(){
    ...
    while(!m_stop){
        ...
        for(int i =0; i<number; i++){
            ...
            //如果是其他可读的数据，那么必然是客户请求到来。调用逻辑处理对象的process方法处理之
    ...
    users = NULL;
    close(pipefd);
    /*close(m_listenfd); 我们将这句话注释掉，以提醒读者：
    应该由m_listenfd的创建者来关闭这个文件描述符（见后文），即所谓的"对象（比如一个文件描述符，又或者一堆段内存）
    由哪个函数创建，就应该由哪个函数销毁"*/
    close(m_epollfd);
}

template<class T>
void process<T>::run_parent(){
    setup_sig_pipe();
    ...
    while(!m_stop){
        number = epoll_wait(e_epollfd, events, MAX_EVENT_NUMBER, -1);
        if( (number<0) &&(errno!=EINTR)){
            printf("epoll failure\n");
            break;
        }
        ...
        for(int i = 0 ; i<number; i++){
            ...
            if(sockfd == m_listenfd){
                //如果由新连接到来，就采用Round Robin 方式将其分配给一个子进程处理
                    ...
                    for(int i =0; i<ret; ++i){
                        switch(signals[i]){
                            ...
                            case SIGINT:{
                                ...
                                当然，通知子进程结束更好的方法是向父子进程之间的通信管道发送
                                特殊数据，读者不妨自己实现之*/
